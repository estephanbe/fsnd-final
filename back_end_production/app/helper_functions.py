...
def sorted_products():
    ...
    try:
        if (sorting is not None and sorting == 'dateN'):
            products = Product.query.order_by(
                Product.creation_date.desc()).all()
        elif (sorting is not None and sorting == 'dateO'):
            products = Product.query.order_by(Product.creation_date).all()
        elif (sorting is not None and sorting == 'priceL'):
            products = Product.query.order_by(Product.price).all()
        elif (sorting is not None and sorting == 'priceH'):
            products = Product.query.order_by(Product.price.desc()).all()
        elif (sorting is not None and sorting == 'ratingL'):
            products = Product.query.order_by(Product.rating).all()
        elif (sorting is not None and sorting == 'ratingH'):
            products = Product.query.order_by(Product.rating.desc()).all()
        elif (sorting is not None and sorting == 'salesL'):
            products = Product.query.order_by(Product.num_of_sales).all()
        elif (sorting is not None and sorting == 'salesH'):
            products = Product.query.order_by(
                Product.num_of_sales.desc()).all()
        else:
...
def sorted_products_by_cat(id):
    ...
    try:
        if (sorting is not None and sorting == 'dateN'):
            products = Product.query.filter_by(cat_id=id).order_by(
                Product.creation_date.desc()).all()
        elif (sorting is not None and sorting == 'dateO'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.creation_date).all()
        elif (sorting is not None and sorting == 'priceL'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.price).all()
        elif (sorting is not None and sorting == 'priceH'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.price.desc()).all()
        elif (sorting is not None and sorting == 'ratingL'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.rating).all()
        elif (sorting is not None and sorting == 'ratingH'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.rating.desc()).all()
        elif (sorting is not None and sorting == 'salesL'):
            products = Product.query.filter_by(
                cat_id=id).order_by(Product.num_of_sales).all()
        elif (sorting is not None and sorting == 'salesH'):
            products = Product.query.filter_by(cat_id=id).order_by(
                Product.num_of_sales.desc()).all()
        else:
...
def sorted_products_by_seller(id):
    ...
    try:
        if (sorting is not None and sorting == 'dateN'):
            products = Product.query.filter_by(seller=id).order_by(
                Product.creation_date.desc()).all()
        elif (sorting is not None and sorting == 'dateO'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.creation_date).all()
        elif (sorting is not None and sorting == 'priceL'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.price).all()
        elif (sorting is not None and sorting == 'priceH'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.price.desc()).all()
        elif (sorting is not None and sorting == 'ratingL'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.rating).all()
        elif (sorting is not None and sorting == 'ratingH'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.rating.desc()).all()
        elif (sorting is not None and sorting == 'salesL'):
            products = Product.query.filter_by(
                seller=id).order_by(Product.num_of_sales).all()
        elif (sorting is not None and sorting == 'salesH'):
            products = Product.query.filter_by(seller=id).order_by(
                Product.num_of_sales.desc()).all()
        else:
